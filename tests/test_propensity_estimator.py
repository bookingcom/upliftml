...
from upliftml.models.h2o import PropensityEstimator
...
def test_train_model(spark: SparkSession, df_h2o_cont: h2o.H2OFrame) -> None:
    ...
    df_h2o_train_eval = df_h2o_cont.cbind(model.predict(df_h2o_cont).set_names(["predicted_propensity"]))
    ...
    # check that the prediction average is close to the true treatment probability
    assert df_pd_train_eval["predicted_propensity"].mean() == pytest.approx(df_h2o_cont[["treatment"]].mean()[0])

    # check that the predicted treatment effect values are between [-1, 1]
    assert df_pd_train_eval["predicted_propensity"].min() >= 0
    assert df_pd_train_eval["predicted_propensity"].max() <= 1


def test_no_model(spark: SparkSession, df_h2o_cont: h2o.H2OFrame) -> None:
    orig_data_types = df_h2o_cont.types
    orig_shape = df_h2o_cont.shape
...
def test_train_model_same_train_val_set(spark: SparkSession, df_h2o_cont: h2o.H2OFrame) -> None:
    orig_data_types = df_h2o_cont.types
    orig_shape = df_h2o_cont.shape
    ...
    model.fit(df_h2o_cont, df_h2o_cont)
...
def test_train_model_val_set(spark: SparkSession, df_h2o_binary: h2o.H2OFrame, df_h2o_binary_val: h2o.H2OFrame) -> None:
    ...
    df_h2o_train_eval = df_h2o_binary.cbind(model.predict(df_h2o_binary).set_names(["predicted_propensity"]))
    df_pd_train_eval = df_h2o_train_eval.as_data_frame()
    ...
    # check that the prediction average is close to the true treatment probability
    assert df_pd_train_eval["predicted_propensity"].mean() == pytest.approx(df_h2o_binary[["treatment"]].mean()[0])
